...
namespace CharGen.Data.Configuration
{
	...
	/// <summary>
	/// NHibernate configuration class for offline mode connections to the data store
	/// </summary>
	public class SQLiteNHibernateConfig : INHibernateConfiguration
	{
		...
		/// <summary>
		/// Initializes a new instance of the <see cref="SQLiteNHibernateConfig" /> class.
		/// </summary>
		/// <param name="config">The configuration.</param>
		public SQLiteNHibernateConfig(IConfiguration config)
		{
		...
		public IPersistenceConfigurer GetConnection()
		{
			var config = SQLiteConfiguration.Standard
				.ConnectionString(c => c.FromConnectionStringWithKey(Configuration.ConnectionName))
				.AdoNetBatchSize(100);
		...
		public FluentConfiguration GetFluentConfiguration()
		{
			var config = Fluently.Configure()
				...
				.Mappings(m =>
				{
					m.HbmMappings.AddFromAssemblyOf<SQLiteNHibernateConfig>();
					m.FluentMappings.AddFromAssemblyOf<SQLiteNHibernateConfig>();
				});
			return config;
		}
		...
		/// </summary>
		/// <returns></returns>
		protected AutoPersistenceModel BuildAutoMap()
		{
			return AutoMap.AssemblyOf<SQLiteNHibernateConfig>()
				.Alterations(alt => alt.AddFromAssemblyOf<SQLiteNHibernateConfig>())
				.UseOverridesFromAssemblyOf<SQLiteNHibernateConfig>()
